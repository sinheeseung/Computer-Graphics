...
def harris_detector(src, k = 0.04, threshold_rate = 0.01, fsize=5):
    ...
    for row in range(h):
        for col in range(w):
            ...
            # R 계산 Harris 방정식 구현
            ##########################################################################
            det_M = M_harris[row, col, 0, 0] * M_harris[row, col, 1, 1] - M_harris[row, col, 1, 0] * M_harris[row, col, 0, 1]
...
def harris_detector_integral(src, k = 0.04, threshold_rate = 0.01, fsize=5):
    ...
    for row in range(h):
        for col in range(w):
            ...
            # R 계산 Harris 방정식 구현
            ##########################################################################
            det_M = M_integral[row, col, 0, 0] * M_integral[row, col, 1, 1] - M_integral[row, col, 1, 0] * M_integral[row, col, 0, 1]
...
def main():
    ...
    cv2.imshow('harris_img ' + '201702033' , harris_img)
    cv2.imshow('harris_integral_img ' + '201702033' , harris_integral_img)
    cv2.waitKey()
